...
class MonoDataset(data.Dataset):
    ...
    def preprocess(self, inputs, color_aug_func):
        ...
        for k in list(inputs):
            ...
            if "color" in k:
                ...
                if n == "color":
                    ...
                    inputs[(n, im, i)] = self.to_tensor(color_aug_func(f))
    ...
    def __getitem__(self, index):
        ...
        for i in self.frame_idxs:
            if i == "s":
                ...
                color_original, color_aug = self.get_color(folder, frame_index, other_side, do_flip, do_crop, crop_params)
            else:
                color_original, color_aug = self.get_color(folder, frame_index + i, side, do_flip, do_crop, crop_params)

            inputs[("color", i, -1)] = color_original
            inputs[("color_aug", i, -1)] = color_aug
        ...
        if do_color_aug:
            color_aug_func = transforms.ColorJitter(
                self.brightness, self.contrast, self.saturation, self.hue)
        else:
            color_aug_func = (lambda x: x)

        self.preprocess(inputs, color_aug_func)
